 ...
 * @author gkspencer
 */
public class JavaFileSearchContext2 extends SearchContext {
	 ...
	 * Class constructor
	 */
	protected JavaFileSearchContext2() {
	}
	...
	protected final String[] getListForDirectory(File file) {
		...
		log4j.debug("getListForDirectory-- File.path="+ file.getAbsolutePath());
		...
		if (fileList == null) {
			...
			File file2 = new File(file.getPath().substring(0,
					file.getPath().length() - 1));
			fileList = file2.list();
		}
		for(int i=0; i<fileList.length; i++){
			if(fileList[i].endsWith("LinkApp.label.xml")){
				File cfile = new File(file.getAbsolutePath()+"/"+fileList[i]);
				if(fileList[i].contains(".LinkApp.label.xml")==false){
				fileList[i] = fileList[i].replaceAll("LinkApp.label.xml", ".LinkApp.label.xml");
				cfile.renameTo(new File(file.getAbsolutePath()+"/"+fileList[i]));
				}
			}
		}
	...
	public boolean hasMoreFiles() {
		log4j.debug("hasMoreFiles -- m_single=" + m_single + " ,m_idx=" + m_idx + ", m_list=" + m_list);
	 ...
	 *            int
	 */
	public final void initPoolSearch(String path, int attr, String[] pathList) throws java.io.FileNotFoundException {
		log4j.debug("initPoolSearch-- path=" + path + " ,attr=" + attr);
		...
		if (pathStr[1] != null
			...
			if (m_root.exists() == false) {
				...
				m_root = new File(FileName.buildPath(pathStr[0], pathStr[1],
						null, java.io.File.separatorChar));
				if (m_root.exists() == false)
	...
	public final void initSearch(String path, int attr) throws java.io.FileNotFoundException {
		log4j.debug("initSearch-- path=" + path + " ,attr=" + attr);
		// Store the search attributes
		...
		if (pathStr[1] != null&& WildCard.containsWildcards(pathStr[1]) == false) {
	...
	public boolean nextFileInfo(FileInfo info) {
		...
		if (isSingleFileSearch()) {
			...
			if (m_idx == 0) {
				...
				log4j.debug("nextFileInfo-- isSingleFileSearch fileName="+ m_root.getName() + " , flen=" + flen + " , FileAttributes=" + fattr);
				...
				long dummyCreate = JavaFileDiskDriver2.getGlobalCreateDateTime();
			...
			while (foundMatch == false && curFile != null) {
				...
				if (m_wildcard.matchesPattern(curFile.getName()) == true) {
					...
					if (FileAttribute.hasAttribute(m_attr,
			...
			if (curFile != null) {
				...
				if (isDirectory(curFile)) {
					...
					if (fname.equalsIgnoreCase("Desktop.ini")
							...
							|| fname.startsWith(".")||fname.equalsIgnoreCase(".LinkApp.label.xml"))
						fattr += FileAttribute.Hidden;
				}
				...
				{
					newfname = fname.replaceAll(".SPARSE.LFS", "");
//					newfname = newfname.replace(newfname, "离线--"+newfname);
					info.setFileName(newfname);
