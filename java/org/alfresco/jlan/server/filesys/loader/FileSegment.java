...
public class FileSegment {
   ...
   * @exception IOException
	 ...
	 * @param tempDir File
	 * @param writeable boolean
	 * @param userPrefix String 用户文件子目录
	 * @return FileSegment
	 * @exception IOException
	 */
	public final static FileSegment createSegment(FileSegmentInfo info, String fname, File tempDir, boolean writeable,String userPrefix)
		throws IOException {
		
		//	Create a temporary file for the data segment
		if(userPrefix != null){
			userPrefix =userPrefix.toLowerCase();
		}
		File file = new File(tempDir+"/"+userPrefix);
		if(file.exists()==false){
			file.mkdir();
		}
		File tempFile = new File(tempDir+"/"+userPrefix+"/", fname);
		info.setTemporaryFile(tempFile.getAbsolutePath());
