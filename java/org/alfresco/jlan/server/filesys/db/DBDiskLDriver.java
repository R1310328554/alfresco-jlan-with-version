...
import com.run.VirtuaServerSyncServlet;
 ...
 */
public class DBDiskLDriver implements DiskInterface, DiskSizeInterface,
		DiskVolumeInterface, NTFSStreamsInterface, FileLockingInterface,
		FileIdInterface, SymbolicLinkInterface, OpLockInterface,
	...
	// Default mode values for files/folders, if not specified in the
	// file/folder create parameters
	...
	// Maximum timestamp value to allow for file timestamps (01-Jan-2030
	// 00:00:00)
	...
	public void closeFile(SrvSession sess, TreeConnection tree, NetworkFile file)
		...
		String shareName = DiskUtil.findFristPath(file.getFullName(), tree.getFullName());
		...
		boolean force = file.isForce();
		boolean archived = file.isArchived();
		boolean closed2 = file.isClosed();
		...
		if (file instanceof DBNetworkFile) {
			...
			// Check if the file state is valid, if not then check the main file
			// state cache
...
//				if (name.startsWith("nsh")&& name.endsWith(".docx")) {
//					log4j.debug("DBDiskLDriver.closeFile()1111 " + name);
//				}
				// DEBUG
				// log4j.debug("** Last file close, no file state for " +
				// file.getFullName());
			} else {
				// If the file open count is now zero then reset the stored
				// sharing mode
				
				int openCount = fstate.getOpenCount();
				LocalFileAccessToken token = (LocalFileAccessToken) file.getAccessToken();
				
				if (token != null) {
					boolean attributesOnly = token.isAttributesOnly();
					int fileAccess = dbCtx.getStateCache().releaseFileAccess(fstate, token);
					
					if (!fstate.isDirectory()) {
						
						DBFileInfo finfo = getFileDetails(fstate.getPath(), dbCtx, fstate,
								userName, shareName, userId);
						boolean b = true;//finfo.isLocked();
						if (b && !fstate.isDirectory()) {// 如果不是目录，而且locked，则检查是否取消其locked权限
							
							if (!fstate.isDirectory() && !file.getName().startsWith("~$")) {
								String ext = DiskUtil.getExt(fstate.getPath());
								if (DBUtil.SUPPORT_EXT.contains(ext) && !ext.equals("txt")) {
									boolean rele = token.isReleased();
									
									int sharedAccess = fstate.getSharedAccess();
									boolean isWps = false;
									if (ext.equals("ppt") || ext.equals("xls") || ext.equals("doc")) {
										isWps = true;
										if (openCount == 1 && !attributesOnly) {
										}
										if (sharedAccess != 7) {
											log4j.debug("DBDiskLDriver.closeFile() sharedAccess != 7 ");
										}
										if (openCount == 2) {
											log4j.debug("DBDiskLDriver.closeFile() openCount == 2 ");
										}
									}
									
									if ((!attributesOnly && openCount == 0) || isWps || (attributesOnly && openCount == 0 && 
											(ext.endsWith("docx") || ext.endsWith("xlsx") || ext.endsWith("pptx")))) {
										
										
										CachedNetworkFile cacheFile = (CachedNetworkFile) file;
										
										FileSegmentInfo info = cacheFile.getFileSegment().getInfo();
										String g = file.getGrantedAccessAsString();
										int status2 = ((DBNetworkFile) file).getStatus();
										
										boolean available = info.isDataAvailable();
										long readableLength = info.getReadableLength();
										int writeCount = file.getWriteCount();
										int status = info.hasStatus();
										boolean queued = info.isQueued();
										boolean updated = info.isUpdated();
										
										
										FileSegment fileSeg = cacheFile.getFileSegment();
										
										if (ext.equals("ppt")) {// 2010 编辑wps ppt
											if (
													
													// 情况1 ，对应打开，然后不做任何修改，直接关闭的情况
													(available && queued == false && updated == false && status == 3 
													&& writeCount == 0 && openCount == 1 && status2 == 0 && g.equals("ReadOnly"))// && status2 == 0 
													
													||
													
													// 情况2 ，对应打开，然后修改，然后关闭的情况:wps的机制，此时要延迟5s左右。
													(available && !queued && !updated && status == 3 
													&& writeCount == 0 && openCount == 1 && status2 == 2 && g.equals("ReadOnly"))) {//
												
//									available && readableLength>0 && status == 3 
//										&& queued == false && status2 == 0 && g.equals("ReadOnly")) {
//									if (available && readableLength>0 && queued == false) {
												MySQLDBLInterface f = (MySQLDBLInterface) dbCtx.getDBInterface();
												f.releaseLock(userId, fstate.getFileId(), shareName);
//										log4j.debug("DBDiskLDriver.closeFile()");
											}
										} else if (ext.equals("doc")) {// wps 编辑wps doc
											if (
													// 情况1 ，对应打开，然后不做任何修改，直接关闭的情况
													(available && queued == false && updated == false // && status == 3 
													&& writeCount == 0 && openCount == 1 && g.equals("WriteOnly"))// && status2 == 0 
													
													||
													
													// 情况2 ，对应打开，然后修改，然后关闭的情况
													(!available && !queued && !updated && status == 0 
													&& writeCount == 0 && openCount == 1 && status2 == 2 && g.equals("WriteOnly"))//
										...
										} else if (ext.equals("xls")) {// wps 编辑wps xls
											if (
													// 情况1 ，对应打开，然后不做任何修改，直接关闭的情况
													(available && queued == false && updated == false && status == 3 
													&& writeCount == 0 && openCount == 1 && g.equals("WriteOnly"))// && status2 == 0 
													...
													// 情况2 ，对应打开，然后修改，然后关闭的情况
													(!available && !queued && !updated && status == 0 
													&& writeCount == 0 && openCount == 1 && g.equals("WriteOnly"))//  && status2 == 2
											...
											if (fileSeg.isUpdated() && file.hasDeleteOnClose() == false) {
												
											} else {
												
												if (available && readableLength>0 && status == 3 && queued == false) {
													MySQLDBLInterface f = (MySQLDBLInterface) dbCtx.getDBInterface();
													f.releaseLock(userId, fstate.getFileId(), shareName);
												}
											}
				...
				if (openCount > 1) {
					...
					// DEBUG
					 log4j.debug("** Last file close, reset shared access for "
					 + file.getFullName() + ", state=" + fstate);
						
				} else {
					// log4j.debug("** File close, file=" + file.getFullName() +
					// ", openCount=" + fstate.getOpenCount());
				}
				...
				if (jdbcFile.hasOpLock()) {
					...
					OpLockManager oplockMgr = flIface.getOpLockManager(sess,
							tree);

					oplockMgr.releaseOpLock(jdbcFile.getOpLock().getPath());

					// DEBUG
					log4j.debug("Released oplock for closed file, file="
							+ jdbcFile.getFullName());
				}
			...
			if (jdbcFile.hasLocks()) {
				...
				// DEBUG
				log4j.debug("Releasing locks for closed file, file="
						+ jdbcFile.getFullName() + ", locks="
			...
			if (fstate != null) {
				...
				DBFileInfo finfo = (DBFileInfo) fstate
						.findAttribute(FileState.FileInformation);
				if (finfo != null && file.getWriteCount() > 0) {
					...
					// DEBUG
					log4j.debug("  File size=" + jdbcFile.getFileSize()
							+ ", modifyDate=" + jdbcFile.getModifyDate());
				}

				// DEBUG
				// log4j.debug("  Open count=" +
				// jdbcFile.getFileState().getOpenCount());
			}

			// Check if the file/directory is marked for delete
			
			/**
			 * 有的doc文档的~$临时文件file.hasDeleteOnClose()为true，有的却false，不清楚为什么。
			 *	file.getName().startsWith("~1111$") && || (file.getWriteCount()==0)
			 */
			if (file.hasDeleteOnClose()) {
				
				boolean b = true;
				
				if (file.hasDeleteOnClose() || b) {
					
					// Check for a file or directory
					if (sharePath.equals(DBUtil.SHARENAME_RECIVEFILE)) {
						// if(shareName.equals(DBUtil.SHARENAME_RECIVEFILE)){
						log4j.warn(DBUtil.SHARENAME_RECIVEFILE
								+ "  中 不允许删除 操作 path:" + file.getFullName());
					...
					// DEBUG
					log4j.debug("  Marked for delete ,fileId:" + file.getFileId()
							+ " ,fullName:" + file.getFullName());
				}
			}
		} else {
			log4j.error("DBD#closeFile() DBNetworkFile fileId:"
					+ file.getFileId() + ", file=" + file);
		}

		// Check if the file was opened for write access, if so then update the
		// file size and modify date/time
		
		
		if (file.getGrantedAccess() != NetworkFile.READONLY
				&& file.isDirectory() == false && file.getWriteCount() > 0) {
			...
			// Check if the modified time is earlier than the file creation
			// date/time
	...
	public void createDirectory(SrvSession sess, TreeConnection tree,
		...
		// Get, or create, a file state for the new path. Initially this will
		// indicate that the directory
		// does not exist.

		FileState fstate = getFileState(params.getPath(), dbCtx, false);
		String shareName = DiskUtil.findFristPath(params.getPath(), tree.getFullName());
		if (fstate != null && fstate.fileExists() == true) {
			log4j.error("Path " + params.getPath() + " exists");
			throw new FileExistsException("Path " + params.getPath()
					+ " exists");
		}
		...
		if (fstate == null) {
			...
			if (getFileDetails(params.getPath(), dbCtx, fstate, userName,
					shareName, userId) != null) {
				log4j.error("Path " + params.getPath() + " exists");
				throw new FileExistsException("Path " + params.getPath()
						+ " exists");
			}
		...
		try {
			...
			if (StringUtils.isEmpty(paths[0]) || "\\".equals(paths[0])) {
				log4j.warn("DBD#createFile() 根目录不允许创建文件 夹, path:"
						+ params.getPath() + " ,path[0]:" + paths[0]);
				...
				if (("\\" + shareName).equalsIgnoreCase(paths[0])) {
					log4j.warn("DBD#createFile() 资料库根目录不允许创建文件夹, path:"
							+ params.getPath());
					throw new AccessDeniedException("资料库根目录不允许创建文件夹");
				}
			} else if (shareName.equalsIgnoreCase(DBUtil.SHARENAME_RECIVEFILE)) {
				log4j.warn("共享文件不允许创建夹");
				throw new AccessDeniedException("共享文件不允许创建夹");
			}
			...
			if (params.hasCreateOption(WinNT.CreateDirectory) == false) {
				...
				throw new IOException(
						"Create directory called for non-directory");
			}

			// Check if the file can be opened in the requested mode
			//
			// Note: The file status is set to NotExist at this point, the file
			// record creation may fail
			...
			fid = dbCtx.getDBInterface().createFileRecord(dname, dirId, params,
					retain, userName, shareName, ipAddress);

			// Indicate that the path exists

			fstate.setFileStatus(FileStatus.DirectoryExists,
					FileState.ReasonFolderCreated);
			...
			if (dbCtx.hasRetentionPeriod() && retain == true) {
				RetentionDetails retDetails = dbCtx.getDBInterface()
						.getFileRetentionDetails(dirId, fid);
				if (retDetails != null)
			...
			if (fid != -1 && dbCtx.getFileLoader() instanceof NamedFileLoader) {
				...
				NamedFileLoader namedLoader = (NamedFileLoader) dbCtx
						.getFileLoader();
			...
			// Check if the file is not valid but an access token has been
			// allocated
	...
	public NetworkFile createFile(SrvSession sess, TreeConnection tree,
		...
		log4j.debug("DBD#createFile(); params.isStream:" + params.isStream()
				+ " ; path:" + params.getPath());
		...
		if (params.isStream()) {
			...
			if (fileExists(sess, tree, params.getPath()) == FileStatus.FileExists) {
				...
				log4j.error("Parent file does not exist to create stream, "
						+ params.getPath());
				throw new FileNotFoundException(
						"Parent file does not exist to create stream, "
								+ params.getPath());
			}
		...
		// Check if the allocation size for the new file is greater than the
		// maximum allowed file size

		if (dbCtx.hasMaximumFileSize()
				&& params.getAllocationSize() > dbCtx.getMaximumFileSize()) {
			log4j.error("Required allocation greater than maximum file size");
			throw new DiskFullException(
					"Required allocation greater than maximum file size");
		}
		...
		try {
			...
			if (StringUtils.isEmpty(paths[0]) || "\\".equals(paths[0])) {
				log4j.warn("DBD#createFile() 根目录不允许创建文件 , path:"
						+ params.getPath() + " ,path[0]:" + paths[0]);
				...
				// 检查是否是公共资料库的第一层
				if (("\\" + shareName).equalsIgnoreCase(paths[0])) {
					log4j.warn("DBD#createFile() 资料库根目录不允许创建文件, path:"
							+ params.getPath());
			...
			if (params.getPath().equals("\\") == false) {
				...
				// Check if the file can be opened in the requested mode
				//
				// Note: The file status is set to NotExist at this point, the
				// file record creation may fail

				int filestate = FileStatus.FileExists;
				// if (fname.endsWith("docx") ||
				// (fname.toLowerCase().startsWith("~") &&
				// fname.endsWith("tmp"))) {
				// filestate = FileStatus.NotExist;
				// } else {
				//
				// }
				accessToken = dbCtx.getStateCache().grantFileAccess(params,
						fstate, FileStatus.NotExist);
			}
			int fid = dbCtx.getDBInterface().createFileRecord(fname, dirId,
					params, retain, userName, shareName, ipAddress);
			// Create a new file record

			// Indicate that the file exists

			fstate.setFileStatus(FileStatus.FileExists,
					FileState.ReasonFileCreated);
			...
			file = (DBNetworkFile) dbCtx.getFileLoader().openFile(params, fid,
					0, dirId, true, false, userName);
	...
	public void deleteDirectory(SrvSession sess, TreeConnection tree, String dir)
		...
		if (shareName.equalsIgnoreCase(DBUtil.SHARENAME_RECIVEFILE)) {
			...
			DBFileInfo dinfo = getFileDetails(dir, dbCtx, fstate, userName,
					shareName, userId);
			if (dinfo == null) {
			...
			try {
				...
				// Check if the file loader handles delete directory requests.
				// Called first as the loader may throw an exception
				// to stop the directory being deleted.

				if (dbCtx.isTrashCanEnabled() == false
						&& dbCtx.getFileLoader() instanceof NamedFileLoader) {
				...
				// Delete the directory file record, or mark as deleted if the
				// trashcan is enabled

				dbCtx.getDBInterface().deleteFileRecord(dinfo.getDirectoryId(),
						dinfo.getFileId(), dbCtx.isTrashCanEnabled(), userName,
						shareName, ipAddress);

				// Indicate that the path does not exist

				fstate.setFileStatus(FileStatus.NotExist,
						FileState.ReasonFolderDeleted);
				fstate.setFileId(-1);
	...
	public void deleteFile(SrvSession sess, TreeConnection tree, String name)
			throws IOException {
		// boolean isWord2003tmp = name.startsWith("~") &&
		// name.toLowerCase().endsWith(".tmp");
		// log4j.debug( "deleteFile : " + name);
		// if (name.endsWith("doc") || name.endsWith("docx")) {// || name.endsWith("tmp")) {
		 // TODO HOW to deal with wps ? wps的文件结尾也是doc啊！
		 // 还有 Excel PowerPoint呢？？
		...
		String ipAddress = sess.getClientInformation().getClientAddress();
		// String shareName = tree.getContext().getShareName();
		String shareName = DiskUtil.findFristPath(name, tree.getFullName());
		...
		if (fstate != null && fstate.fileExists() == false) {
			if (StringUtils.isEmpty(ext) || "tmp".equals(ext)) {
				log4j.warn("DBD#deleteFile() 未产生的临时文件不返回 File does not exist  name:"
						+ name);
				// return ;// 不往下执行
			} else {
				log4j.warn("File does not exist, " + name);
			}
			throw new FileNotFoundException("File does not exist, " + name); //此处似乎取消注释也没关系,因为后面的执行不影响缓存/数据库
		}
		...
		try {
			...
			dbInfo = getFileDetails(name, dbCtx, fstate, userName, shareName, userId);
			...
			/*
			 * 按条件不执行删除：(office改名删除的情况) start int userId = 0; try{ UserBean user
			 * = dbCtx.getDBInterface().getUserByUsername(userName); if(null !=
			 * user) { userId = user.getId(); } } catch (DBException ex) {
			 * log4j.
			 * error("User is not exist from database, userId:"+userId+" username:"
			 * +userName,ex); throw new
			 * AccessDeniedException("User is not exist from database, userId:"
			 * +userId+" username:"+userName,ex); } DBFileInfo finfo =
			 * dbCtx.getDBInterface().getFileInformation(0, fstate.getFileId(),
			 * DBInterface.FileNameOnly,userId,shareName,userName); if(null !=
			 * finfo) { String[] paths = FileName.splitPath( name); String fname
			 * = paths[1]; if(!finfo.getFileName().equalsIgnoreCase(fname)) {
			 * //名称不同，不执行删除 return ;//不执行下面的操作 } if(finfo.isReadOnly()) {
			 * log4j.warn(fstate.getFileId()+" ,只读文件"); throw new
			 * AccessDeniedException("只读文件"); } }
			 */
			/* 按条件不执行删除：(office改名的情况) end */
			// Delete the file in the filesystem/repository, the loader may
			// prevent the file delete by throwing
			// an exception
			...
			if (null != dbInfo && dbInfo.isFileType() == FileType.SymbolicLink
					&& canDel)
				dbCtx.getDBInterface().deleteSymbolicLinkRecord(
						dbInfo.getDirectoryId(), fstate.getFileId());

			/*
			 * 这部分可能导致保存后，WPS保存doc及时 (加上这一段Office2007编辑时会报错） // Check if the
			 * file has any NTFS streams
			 */
			StreamInfoList streamList = getStreamList(sess, tree, name);

			if (streamList != null && streamList.numberOfStreams() > 0) {

				// Make a copy of the streams list as streams are removed from
				// the original list as we delete them
			...
			if (null != dbInfo && null != fstate) {
				dbCtx.getDBInterface().deleteFileRecord(
						dbInfo.getDirectoryId(), fstate.getFileId(),
						dbCtx.isTrashCanEnabled(), userName, shareName,
						ipAddress);
			}
			...
			// if("tmp".equals(ext)|| "xls".equals(ext) || "xlsx".equals(ext) ||
			// (name.startsWith("~$") && "doc".equals(ext)) ||
			// (name.startsWith("~$") && "docx".equals(ext)))
			// if("tmp".equals(ext) || "xls".equals(ext) ||
			// (name.startsWith("~$") &&
			// "doc".equals(ext))||(name.startsWith("~$") &&
			// "xlsx".equals(ext)))
			// if("tmp".equals(ext) || "xls".equals(ext) ||
			// (name.startsWith("~$") && "doc".equals(ext))
			...
			// {
			fstate.setFileStatus(FileStatus.NotExist,
					FileState.ReasonFileDeleted);
			fstate.setFileId(-1);
//			log4j.warn("DBD#deleteFile() FileStatus.NotExist-fstate.setFileId(-1): name"
//					+ name + " , ext:" + ext);
			// }
			fstate.removeAttribute(FileState.FileInformation);

			// Check if there is a quota manager, if so then release the file
			// space
			...
			if (dbCtx.hasQuotaManager() && null != dbInfo) {
				...
				dbCtx.getQuotaManager().releaseSpace(sess, tree,
						fstate.getFileId(), null, dbInfo.getSize());
			}
		} catch (DBException ex) {
	...
	public int fileExists(SrvSession sess, TreeConnection tree, String name) {
		...
		if (FileName.containsStreamName(name)) {
			...
			// Debug
			log4j.debug("DBD#fileExists() nameWithStream=" + name
					+ ", fileSts=" + FileStatus.asString(fileSts));
		} else {
			...
			if (fstate.getFileStatus() == FileStatus.Unknown
					|| (fstate.getFileStatus() == FileStatus.NotExist && DBUtil.SUPPORT_EXT
							.contains(ext))) {
				// fstate.getFileStatus()==FileStatus.NotExist
				// &&DBUtil.SUPPORT_EXT.contains(ext) ā
				...
				// Debug
				// log4j.debug("DBD#fileExists() name=" + name + ", fileSts=" +
				// FileStatus.asString(fileSts));
			} else {

				// DEBUG
				// log4j.debug("@@ Cache hit - fileExists() name=" + name +
				// ", fileSts=" + FileStatus.asString(fileSts));
			}
		}
	...
	public FileInfo getFileInformation(SrvSession sess, TreeConnection tree,
		...
		if (FileName.containsStreamName(name)) {
			...
			if (finfo == null) {
				...
				// Get the file information for the parent file to load the
				// cache

				if (parent == null) {

					// Get the file information for the parent file
					log4j.debug("parent is null 再次调用:getFileInformation() ; filepath:"
							+ filePath + ", finfo:" + finfo);
				...
				if (parent != null && parent.fileExists() == true) {
					...
					if (dbInfo != null) {
						...
						StreamInfoList streams = getStreamList(sess, tree,
								filePath);

						if (streams != null && streams.numberOfStreams() > 0) {

							// Parse the path into directory, file and stream
							// names

							String[] paths = FileName.splitPathStream(name);

							// Get the details for the stream, if the
							// information is valid copy it to a file
							// information
							// object
							...
							if (sInfo != null) {
								...
								// Create a file information object, copy the
								// stream details to it

								finfo = new DBFileInfo(paths[1], name,
										dbInfo.getFileId(),
										dbInfo.getDirectoryId());

								finfo.setFileId(sInfo.getFileId());
								finfo.setFileSize(sInfo.getSize());

								finfo.setCreationDateTime(sInfo
										.getCreationDateTime());
								finfo.setAccessDateTime(sInfo
										.getAccessDateTime());
								finfo.setModifyDateTime(sInfo
										.getModifyDateTime());

								// Attach to the file state

								fstate.addAttribute(FileState.FileInformation,
										finfo);

								// DEBUG
								log4j.debug("DBD#getFileInformation() stream="
										+ name + ", info=" + finfo);
							}
						}
					}
			...
			DBFileInfo dbInfo = getFileDetails(name, dbCtx, fstate, userName,
					shareName, userId);
...
//			// TODO
//			boolean b = finfo.hasAttribute(FileAttribute.ReadOnly);
//			if (!b) {
//				DBFileInfo f = (DBFileInfo) finfo;
//				boolean open = VirtuaServerSyncServlet.isFileOpen(userId, f.getFullName(), shareName);
//				if (open) {
//					int at = f.getFileAttributes();
//					at += FileAttribute.ReadOnly;
//					f.setFileAttributes(at);
//				}
//			}
//		}
	...
	public NetworkFile openFile(SrvSession sess, TreeConnection tree,
		...
		// Check if the file has a data update in progress, the file will be
		// offline until the
...
//		if (name.startsWith("nsh")&& name.endsWith(".docx")) {
//			log4j.debug("DBDiskLDriver.oooo00000 " + name + " / " + fstate.getOpenCount());
//		}
		...
		if (dbCtx.hasRetentionPeriod()) {
			try {
				...
				RetentionDetails retDetails = dbCtx.getDBInterface()
						.getFileRetentionDetails(finfo.getDirectoryId(),
								finfo.getFileId());
				if (retDetails != null)
					fstate.setRetentionExpiryDateTime(retDetails.getEndTime());
			} catch (DBException ex) {
				log4j.error("DBD#Retention error, " + ex.getMessage());
				throw new AccessDeniedException("Retention error, "
						+ ex.getMessage());
			}
		}
		...
		if (params.getPath().equals("\\") == false) {
			...
			accessToken = dbCtx.getStateCache().grantFileAccess(
					params,
					fstate,
					finfo.isDirectory() ? FileStatus.DirectoryExists
							: FileStatus.FileExists);
		}

		// DEBUG
		// log4j.debug("DBD#openFile() name=" + params.getPath() +
		// ", sharing=0x" + Integer.toHexString(params.getSharedAccess()) +
		// ", PID=" + params.getProcessId() + ", token=" + accessToken);
		...
		try {
			...
			jdbcFile = (DBNetworkFile) dbCtx.getFileLoader().openFile(params,
					finfo.getFileId(), 0, finfo.getDirectoryId(), false,
					finfo.isDirectory(), userName);

			jdbcFile.setFileDetails(finfo);
			jdbcFile.setFileState(dbCtx.getStateCache().getFileStateProxy(
					fstate));
			...
			if (!params.isDirectory()) {// 如果不是目录，而且非locked，则检查是否给予其locked权限
				boolean b = finfo.isLocked();
				String ext = DiskUtil.getExt(fname);
				if (!b && ext != null && DBUtil.SUPPORT_EXT.contains(ext) && !ext.equals("txt") && !fname.startsWith("~$")) {
					
					if (!params.isAttributesOnlyAccess() && params.isReadOnlyAccess()) {
						int createOptions = params.getCreateOptions();
						int openCount = fstate.getOpenCount() ;
						
						//ext.equals("xls") || 
						if (ext.equals("doc")) {// wps 编辑wps doc // 2010 打开doc后解锁有问题。。,但是wps就ok
							//						if (openCount == 2) {
							

							if (createOptions == 64 && openCount == 3) {
								MySQLDBLInterface f = (MySQLDBLInterface) dbCtx.getDBInterface();
								f.lock(userId, finfo.getFileId(), shareName);
							}
						} else if (ext.equals("ppt")) {// wps 编辑wps ppt	 && access == 3 ,解锁过快，， // 2010 打开xls后解锁没有问题。但加锁有问题。。

							if (createOptions == 64 && openCount == 2) {
								MySQLDBLInterface f = (MySQLDBLInterface) dbCtx.getDBInterface();
								f.lock(userId, finfo.getFileId(), shareName);
							}
							
						} else if (ext.equals("xls")) {// wps 编辑wps xls  // 2010 打开xls后解锁有问题。。,但是wps就ok
							
							if (createOptions == 64 && openCount == 2) {
								MySQLDBLInterface f = (MySQLDBLInterface) dbCtx.getDBInterface();
								f.lock(userId, finfo.getFileId(), shareName);
							}
						} else if (createOptions > 64 && openCount == 2) {
			...
			// If the file object is not valid then release the file access that
			// was granted
	...
	public int readFile(SrvSession sess, TreeConnection tree, NetworkFile file,
			byte[] buf, int bufPos, int siz, long pos) throws IOException {

		// Debug
		// log4j.debug("DBD#readFile() fileId:"+file.getFileId()+",name:"+file.getName()+",fileSize:"+file.getFileSize()+",filePos="
		// + pos + ", len=" + siz);
		// Access the JDBC context
		...
		if (file instanceof DBNetworkFile) {
			...
			if (jfile.hasFileState() && jfile.getFileState().hasActiveLocks()) {
				...
				// Check if this session has write access to the required
				// section of the file

				if (jfile.getFileState().canReadFile(pos, siz,
						sess.getProcessId()) == false) {
	...
	public void renameFile(SrvSession sess, TreeConnection tree,
		...
		String ipAddress = sess.getClientInformation().getClientAddress();
		// String shareName = tree.getContext().getShareName();
		log4j.debug("DBD#renameFile() ,time:" + System.currentTimeMillis()
		...
		if (oldName.equalsIgnoreCase("\\" + DBUtil.SHARENAME_USERFILE)
				|| oldName.equalsIgnoreCase("\\" + DBUtil.SHARENAME_RECIVEFILE)
				|| oldName.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE)
		...
		if (paths.length == 2) {
			...
			if (firstPath.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE)
			  ||firstPath.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE_ALIAS)) {
				throw new AccessDeniedException("资料库直接子目录不允许改名或移动");
			}
		}
		...
		String shareName = DiskUtil.findFristPath(oldName, tree.getFullName());
		...
		try {
			...
			if (newFname != null && newFname.length() > MaxFileNameLen) {
				...
				throw new FileNameException("Destination name too long, "
						+ newFname);
			}
			...
			if (fid == -1) {
				...
				// Split the current path string and find the file id of the
				// existing file/directory

				dirId = findParentDirectoryId(dbCtx, oldName, true, userName,
						shareName, userId);
				if (dirId == -1) {
					log4j.error("DBD#renameFile() dirId==-1 FileNotFoundException ,name:"
							+ oldName);
				...
				fid = getFileId(oldName, oldFname, dirId, dbCtx, userId,
						shareName, userName);
				if (fid == -1) {
					log4j.error("DBD#renameFile() fid==-1 FileNotFoundException ,name:"
							+ oldName);
					throw new FileNotFoundException(oldName);
				}
			...
			int newDirId = findParentDirectoryId(dbCtx, newName, true,
					userName, shareName, userId);
			if (newDirId == -1) {
				log4j.error("DBD#renameFile() newDirId==-1 FileNotFoundException , name:"
						+ newName);
				throw new FileNotFoundException(newName);
			}

			// Rename the file/folder, this may also link the file/folder to
			// a new parent directory
			int newFid = 0;
			if (curInfo != null) {
				if (curInfo.isDirectory()) { // 此时的fid 就是一个目录id
					//int uid = sess.getClientInformation().getUid();
					if (!newFname.equals(oldFname)) {// 此处对应的是直接重命名	
						newFid = (int) dbCtx.getDBInterface().renameFolderRecord(newDirId, -1,
								fid, curInfo.getUid(), newFname, shareName, ipAddress);
						
					} else { // 此处对应的是目录剪切						
						newFid = (int) dbCtx.getDBInterface().renameFolderRecord(newDirId, dirId,
								fid, curInfo.getUid(), newFname, shareName, ipAddress);
					}
				} else {
					if (!newFname.equals(oldFname)) {
						newFid = dbCtx.getDBInterface().renameFileRecord(dirId,
								fid, newFname, newDirId, shareName, ipAddress);
					} else { // 此处对应的是文件剪切
						newFid = (int) dbCtx.getDBInterface().renameFolderRecord(newDirId, dirId,
								fid, curInfo.getUid(), newFname, shareName, ipAddress);
					}
			...
			if (null != fileSegInfo) {
				...
				// 对于新建的xls: 新建 Microsoft Excel 工作表.xls 改名--或者简单的xls重命名， 不需要执行下面的方法
//				if (oldFname.endsWith(".xls") && newFname.endsWith(".xls")) {
//					
//				} else
//					dbCtx.getDBInterface().modifyFileTemporaryFile(newFid,
//						fileSegInfo, shareName);// 修改上传的null路径为临时文件路径
			}
			if (newFid != fid) {
				log4j.debug("gonna to reset the fid in the state ,this is very important !!!! ");
				
				if (dbCtx.getFileLoader() instanceof NamedFileLoader) {
					// Rename the file/directory
					NamedFileLoader namedLoader = (NamedFileLoader) dbCtx
							.getFileLoader();
					namedLoader.renameFileDirectory(oldName, newFid,
							newName, curInfo.isDirectory());
				}
				...
				if (DBUtil.SUPPORT_EXT.contains(ext)) {
					VirtuaServerSyncServlet.notifyOthers(userId, newDirId, newFid, newName, newFname, shareName);
				}
				
//				log4j.debug("DBDiskLDriver.renameFile()");
//				fstate.setExpiryTime(0);
				/*
				 * 这部分可能导致保存后，提示权限不对。暂不执行 //删除原来的 fstate.setFileStatus(
				 * FileStatus.NotExist, FileState.ReasonFileDeleted);
				 * fstate.setFileId(-1);
				 * fstate.removeAttribute(FileState.FileInformation); fstate
				 * = getFileState(newFname, dbCtx, true);//重新获得对象 //创建
				 * fstate.setFileStatus( FileStatus.FileExists,
				 * FileState.ReasonFileCreated); // Save the file id
				 * fstate.setFileId(newFid);
				 */
			} else {
				if (dbCtx.getFileLoader() instanceof NamedFileLoader) {
					...
					NamedFileLoader namedLoader = (NamedFileLoader) dbCtx
							.getFileLoader();
					namedLoader.renameFileDirectory(oldName, fid, newName,
							curInfo.isDirectory());
				}
			 ...
			 * TODO needed ?
			 */
			dbCtx.getStateCache().renameFileState(newName, fstate,
					curInfo.isDirectory());
			fstate.removeAllAttributes();
			
		} catch (DBException ex) {
			log4j.error("DBD#renameFile() FileNotFoundException , name:"
					+ oldName + " , newName:" + newName, ex);
			throw new FileNotFoundException(oldName);
		}
	}
	...
	public void setFileInformation(SrvSession sess, TreeConnection tree,
		...
		int userId = sess.getClientInformation().getUid();
		// String shareName = tree.getContext().getShareName();
		log4j.debug("DBD#setFileInformation() name=" + name + ", info=" + info
				+ ", set flags=" + info.getSetFileInformationFlagsString());

		// If the only flag set is the delete on close flag then return, nothing
		// to do
		...
		try {
			...
			// Check if the loader handles set file information requests, an
			// exception may be thrown by the loader
			...
			//
			// Switch off invalid updates from being written to the database but
			...
			if (info.hasSetFlag(FileInfo.SetAccessDate)
					&& info.getAccessDateTime() > MaxTimestampValue)
				dbFlags -= FileInfo.SetAccessDate;

			if (info.hasSetFlag(FileInfo.SetCreationDate)
					&& info.getCreationDateTime() > MaxTimestampValue)
				dbFlags -= FileInfo.SetCreationDate;

			if (info.hasSetFlag(FileInfo.SetModifyDate)
					&& info.getModifyDateTime() > MaxTimestampValue)
			...
			if (info.hasChangeDateTime() == false) {
				...
				if (info.hasSetFlag(FileInfo.SetChangeDate) == false)
					info.setFileInformationFlags(info
							.getSetFileInformationFlags()
							+ FileInfo.SetChangeDate);
			} else if (info.hasSetFlag(FileInfo.SetChangeDate)
					&& info.getChangeDateTime() > MaxTimestampValue)
			...
			if (info.hasSetFlag(FileInfo.SetAttributes)) {
				...
				// Check if this is a folder, make sure the Directory attribute
				// does not get reset

				if (dbInfo.isDirectory()
						&& (info.getFileAttributes() & FileAttribute.Directory) == 0)
					info.setFileAttributes(info.getFileAttributes()
			...
			if (dbFlags != 0)
				dbCtx.getDBInterface().setFileInformation(
						dbInfo.getDirectoryId(), dbInfo.getFileId(), info,
						shareName);

			// Use the original information flags when updating the cached file
			// information details
	...
	public SearchContext startSearch(SrvSession sess, TreeConnection tree,
		...
		// if(StringUtils.isNotEmpty(searchPath) && !searchPath.equals("\\") &&
		// !searchPath.equals("\\*") &&
		// searchPath.toLowerCase().startsWith("\\"+userName.toLowerCase()+DBUtil.SPECIAL_CHAR)==false)
		// {
		// log4j.warn("DBD#startSearch  不符合规则的searchPath值!  searchPath:"+searchPath);
		// return null;
		// }
		...
		int dirId = findParentDirectoryId(dbCtx, searchPath, true, userName,
				shareName, userId);
		if (dirId == -1) {
		...
		try {
			...
			// Check if the search path is a none wildcard search, the file
			...
			if (WildCard.containsWildcards(searchPath) == false) {
				...
				if (searchState != null && searchState.fileExists() == true) {
					...
					DBFileInfo finfo = (DBFileInfo) searchState
							.findAttribute(FileState.FileInformation);

					if (finfo != null) {

						// Create a single file search context using the cached
						// file information
						...
						// DEBUG
						// log4j.debug("DBD#StartSearch using cached file information, path="
						// + searchPath + ", info=" + finfo);
					}
				}
			}

			// Start the search via the database interface, if the search is not
			// valid

			if (search == null) {

				if (dirId <= 0
						&& searchPath.indexOf("\\", 2) == -1
						&& tree.getContext().getShareName()
								.indexOf(DBUtil.SPECIAL_CHAR) > 0) {
					// 第一层加cache特殊处理
					CacheManager cacheManager = CacheManagerUtil
							.getCacheManager();
					Cache cache = null;
					...
					if (null != cacheManager) {
						...
						if (null != element && element.getObjectValue() != null) {
							...
							if (null != search && search.getMaximumFiles() > 0) {
								log4j.debug("@@ Cache hit - SearchSharesCache :"
										+ searchPath);
								return search;
							}
						}
					...
					DBSearchContext dbSearch = dbCtx.getDBInterface()
							.startSearch(dirId, searchPath, attrib,
									DBInterface.FileAll, -1, userName,
									shareName);
				...
				DBSearchContext dbSearch = dbCtx.getDBInterface().startSearch(
						dirId, searchPath, attrib, DBInterface.FileAll, -1,
						userName, shareName);
	...
	public void truncateFile(SrvSession sess, TreeConnection tree,
			NetworkFile file, long siz) throws java.io.IOException {
		...
		if (file instanceof DBNetworkFile) {
			...
			if (jfile.isStream()) {
				log4j.warn("DBD#truncateFile(),附加流不截断 fullName:"
						+ jfile.getFullName());
				return;
			}
			...
			DBFileInfo dbInfo = getFileDetails(jfile.getFullName(), dbCtx,
					fstate, userName, shareName, userId);
			if (dbInfo == null) {
				log4j.error("FileNotFoundException ,fullName:"
						+ jfile.getFullName());
				throw new FileNotFoundException(jfile.getFullName());
			}

			// Check if the new file size is greater than the maximum allowed
			...
			if (dbCtx.hasMaximumFileSize() && siz > dbCtx.getMaximumFileSize()) {
				...
				throw new DiskFullException(
						"Write is beyond maximum allowed file size");
			}

			// Keep track of the allocation/release size in case the file resize
			// fails
			...
			if (dbCtx.hasQuotaManager()) {
				...
				// Determine if the new file size will release space or require
				// space allocating
				...
				if (siz > dbInfo.getAllocationSize()) {
					...
					// Calculate the space to be released as the file is to be
					// truncated, release the space if
			...
			try {
				...
				if (allocSize > 0 && quotaMgr != null)
					quotaMgr.releaseSpace(sess, tree, file.getFileId(), null,
							allocSize);
			...
			if (allocSize > 0)
				...
				dbInfo.setAllocationSize(dbInfo.getAllocationSize()
						- releaseSize);
			...
			try {
				...
				dbCtx.getDBInterface().setFileInformation(
						jfile.getDirectoryId(), jfile.getFileId(), finfo,
						shareName);
	...
	public int writeFile(SrvSession sess, TreeConnection tree,
		...
		if (file.isStream()) {
			log4j.warn("DBD#writeFile() NTFS Alternate Data Streams 交换数据流 ，不做保存处理 , fid:"
					+ file.getFileId()
					+ ",name:"
					+ file.getFullName()
					+ " , siz:" + siz + ", fileoff:" + fileoff);
			// return 0;//报错
			return siz;
		}
		...
		if (file instanceof DBNetworkFile) {
			...
			if (jfile.hasFileState() && jfile.getFileState().hasActiveLocks()) {
				...
				if (jfile.getFileState().canWriteFile(fileoff, siz,
						sess.getProcessId()) == false)
					throw new LockConflictException();
			}

			// Check if there is a maximum file size, if so then check if the
			// write is beyond the allowed file size

			if (dbCtx.hasMaximumFileSize()
					&& (fileoff + siz) > dbCtx.getMaximumFileSize()) {
			...
			if (quotaMgr != null) {
				...
				DBFileInfo finfo = getFileDetails(jfile.getFullName(), dbCtx,
						jfile.getFileState(), userName, shareName, userId);
				if (finfo == null) {
					log4j.error("FileNotFoundException name:"
							+ jfile.getFullName());
					throw new FileNotFoundException(jfile.getFullName());
				...
				try {
					...
					if (extendSize > 0 && quotaMgr != null)
						quotaMgr.releaseSpace(sess, tree, file.getFileId(),
								null, extendSize);
				...
				if (extendSize > 0)
					finfo.setAllocationSize(MemorySize
							.roundupLongSize(endOfWrite));
			} else {
	...
	/**
	 * Parse/validate the parameter string and create a device context for this
	 * share
	 * 
	 * @param shareName
	 *            String
	 ...
	 * @return DBFileInfo
	 */
	protected final DBFileInfo getFileDetails(int userId, String path, DBDeviceContext dbCtx) {

		return getFileDetails(path, dbCtx, null, null, dbCtx.getShareName(), userId);
	}
	 ...
	 * @return DBFileInfo
	 */
	protected final DBFileInfo getFileDetails(String path,
			DBDeviceContext dbCtx, FileState fstate, String userName,
			String shareName, int userId) {
		if (path.endsWith(".dll")||path.endsWith(".wbk")) {
		...
		if (path.length() == 0 || path.compareTo("\\") == 0) {
			...
			// log4j.debug("DBD#getFileDetails 返回root目录   fileId:"+rootDir.getFileId()+" ,fileName:"
			// + rootDir.getFileName());
		...
		if (path.equalsIgnoreCase("\\" + userName + DBUtil.SPECIAL_CHAR)) // 新增加
		{
			...
			// log4j.debug("DBD#getFileDetails 返回root目录   fileId:"+rootDir.getFileId()+" , userName:"+userName+" ,fileName:"
			// + rootDir.getFileName());
		...
		// 增加根路径的直接返回目录存在(如:username@/我的文件; username@/资料库)
		//
		if (path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_USERFILE)
				|| path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_RECIVEFILE)
				|| path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE)
				|| path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE_ALIAS)) {
			DBFileInfo rootDir = dbCtx.getRootDirectoryInfo();
			rootDir.setFileId(0);
			rootDir.setPath(path);
			rootDir.setFileName(path);
		...
		int dirId = findParentDirectoryId(dbCtx, path, true, userName,
				shareName, userId);
		if (dirId == -1) {
		...
		if (paths[0] != null
				&& paths[0].endsWith(FileName.DOS_SEPERATOR_STR) == false)
		...
		int fid = getFileId(filePath, fname, dirId, dbCtx, userId, shareName,
				userName);
		if (fid == -1) {
		...
		DBFileInfo finfo = getFileInfo(filePath, dirId, fid, dbCtx, userName,
				shareName, userId);
		...
		if (finfo != null && fstate != null) {
			...
			fstate.setFileStatus(finfo.isDirectory() ? FileStatus.DirectoryExists
					: FileStatus.FileExists);
			...
			if (dbCtx.hasOfflineFiles()
					&& finfo.hasAttribute(FileAttribute.NTOffline) == false) {
				if (dbCtx.getOfflineFileSize() == 0
						|| finfo.getSize() >= dbCtx.getOfflineFileSize())
					finfo.setFileAttributes(finfo.getFileAttributes()
							+ FileAttribute.NTOffline);
			}
	 ...
	 * @return int
	 */
	protected final int getFileId(String path, String name, int dirId,
			DBDeviceContext dbCtx, int userId, String shareName, String userName) {

		if (path.equalsIgnoreCase("\\" + userName + DBUtil.SPECIAL_CHAR)
				|| path.equalsIgnoreCase("\\" + userName)) // 新增加
		{
			return 0;// \admin@\
		}
		// 上面的为旧的方式，已经不适用
		// 增加根路径的直接返回目录存在(如:username@/我的文件; username@/资料库)
		//
		if (path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE)
				|| path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_USERFILE)
				|| path.equalsIgnoreCase("\\" + DBUtil.SHARENAME_RECIVEFILE)) // 新增加
		{
			return 0;// \admin@\
		}
...
//							+ " , shareName:" + shareName);
				...
				if (state.getFileId() != -1) {
					...
					// DEBUG
					//log4j.debug("@@ Cache hit - getFileStatus() name=" + name
					//		+ ", sts=NotExist");
...
//			log4j.debug();
//			log4j.debug();
//			log4j.debug("DBDiskLDriver.getFileId()");
//			log4j.debug();
//			log4j.debug();
			
			fileId = dbCtx.getDBInterface().getFileId(dirId, name, false,
					false, userId, shareName, userName);
		} catch (DBException ex) {
		}
	 ...
	 */
	protected final void getRetentionDetailsForState(DBDeviceContext dbCtx,
			FileState fstate) throws DBException {
		...
		if (dbCtx.hasRetentionPeriod()) {
			...
			RetentionDetails retDetails = dbCtx.getDBInterface()
					.getFileRetentionDetails(-1, fstate.getFileId());
			if (retDetails != null)
				fstate.setRetentionExpiryDateTime(retDetails.getEndTime());
		}
	}
	 ...
	 * @return int
	 */
	protected final int findParentDirectoryId(DBDeviceContext ctx, String path,
			boolean filePath, String userName, String shareName, int userId) {

		if (null != path
				&& path.equalsIgnoreCase("\\" + userName + DBUtil.SPECIAL_CHAR))// 用户根目录
			return 0;
		// 上面的为旧的方式，已经不适用
		// 增加根路径的直接返回目录存在(如:username@/我的文件; username@/资料库)
		//
		...
		if (paths[0] != null && paths[0].compareTo("\\") == 0
		...
		if (cache != null) {
			...
			if (state != null && state.getFileId() != -1) {
				...
				// log4j.debug("@@ Cache hit - findParentDirectoryId() path=" +
				// paths[0]);
		...
		int[] ids = findParentDirectoryIdList(ctx, path, filePath, userName,
				shareName, userId);
		if (ids == null) {
	 ...
	 * @return int[]
	 */
	protected final int[] findParentDirectoryIdList(DBDeviceContext ctx,
			String path, boolean filePath, String userName, String shareName,int userId) {
		// log4j.debug("DBD#findParentDirectoryIdList() path:"
		// +path+" , filePath:"+filePath+" ,userName:"+userName);
		...
		if (paths[0].compareTo("*.*") == 0 || paths[0].compareTo("*") == 0
				...
				|| path.equals("\\" + userName + DBUtil.SPECIAL_CHAR)) {
			int[] ids = { 0 };
			return ids;
		}
		// 上面的为旧的方式，已经不适用
		...
		try {
			...
			while (idx <= endIdx) {
				...
				// log4j.debug("DBD#findParentDirectoryIdList() idx:"
				// +idx+" , curPath:"+curPath+" ,pathStr:"+pathStr+", parentId:"+parentId+",dirId:"+dirId);
				...
				if (fstate != null && fstate.getFileId() != -1) {
					...
					dirId = ctx.getDBInterface().getFileId(dirId, curPath,
							true, true, userId, shareName, userName);
					...
					if (dirId != -1) {
						...
						// Check if we have a file state, or create a new file
						...
						if (fstate != null) {
							...
							DBFileInfo finfo = ctx.getDBInterface()
									.getFileInformation(parentId, dirId,
											DBInterface.FileAll, userId,
											shareName, userName);
							fstate.addAttribute(FileState.FileInformation,
									finfo);
							fstate.setFileStatus(finfo.isDirectory() ? FileStatus.DirectoryExists
	...
	/**
	 * Return file information about the specified file, using the internal file
	 * id
	 * 
	 * @param path
	 *            String
	 ...
	 * @exception IOException
	 */
	public DBFileInfo getFileInfo(String path, int dirId, int fid,
			DBDeviceContext dbCtx, String userName, String shareName, int userId) {
		...
		if (state != null && state.getFileId() != -1) {
			...
			DBFileInfo finfo = (DBFileInfo) state
					.findAttribute(FileState.FileInformation);
			if (finfo != null)
				return finfo;
		}
		...
		try {
			...
			finfo = dbCtx.getDBInterface().getFileInformation(dirId, fid,
					DBInterface.FileAll, userId, shareName, userName);
			// log4j.debug("DBD#getFileInfo ; dirId:" +
			// dirId+" , fid:"+fid+", userId: "+userId+", shareName :"+shareName);
		} catch (DBException ex) {
			log4j.error(ex);
			finfo = null;
		}
		...
		if (state != null && finfo != null) {
			...
			state.setFileStatus(finfo.isDirectory() ? FileStatus.DirectoryExists
					: FileStatus.FileExists);
		}
	 ...
	 * @return DBFileInfo
	 */
	public DBFileInfo getStreamInfo(FileState parent, String[] paths,
		...
		log4j.debug("DBD#getStreamInfo parent=" + parent.getPath()
				+ ", stream=" + paths[2]);
		...
		try {
			...
			StreamInfoList sList = (StreamInfoList) parent
					.findAttribute(DBStreamList);

			if (sList == null) {

				// No cached stream information, get the list from the database

				sList = dbCtx.getDBInterface().getStreamsList(
						parent.getFileId(), DBInterface.StreamAll);
	 ...
	 * @return FileState
	 */
	protected final FileState getFileState(String path, DBDeviceContext ctx,
			boolean create) {
	...
	public final void getDiskInformation(DiskDeviceContext ctx, SrvDiskInfo info)
		...
		if (ctx instanceof DBDeviceContext) {
			...
			// Access the associated file loader class, if it implements the
			// disk size interface then call the loader
			...
			if (dbCtx.getFileLoader() instanceof DiskSizeInterface) {
				...
				DiskSizeInterface sizeInterface = (DiskSizeInterface) dbCtx
						.getFileLoader();
		...
		// Check if there is a quota manager, if so then get the current free
		...
		if (ctx.hasQuotaManager()) {
			...
			// DEBUG
			log4j.debug("DBD#getDiskInformation() freeSpace:" + freeSpace
					+ " , freeUnits:" + freeUnits);
		}
	}

	/**
	 * Determine if NTFS streams support is enabled. Check if the associated
	 * file loader supports NTFS streams.
	 * 
	 * @param sess
	 *            SrvSession
	 * @param tree
	...
	public boolean hasStreamsEnabled(SrvSession sess, TreeConnection tree) {
		...
		if (tree.getContext() instanceof DBDeviceContext) {
			...
			// Access the associated file loader class to check if it supports
			// NTFS streams
	 ...
	 * @exception IOException
	 */
	public StreamInfo getStreamInformation(SrvSession sess,
			TreeConnection tree, StreamInfo streamInfo) throws IOException {
	 ...
	 * @exception IOException
	 */
	public StreamInfoList getStreamList(SrvSession sess, TreeConnection tree,
		...
		// DEBUG
		log4j.debug("DBD#getStreamList() fileName=" + fileName + ", userName:"
				+ userName);

		// Check if the file state already has the streams list cached

		StreamInfoList streamList = (StreamInfoList) fstate
				.findAttribute(DBStreamList);
		if (streamList != null && streamList.numberOfStreams() > 0)
			return streamList;
		...
		DBFileInfo finfo = getFileDetails(fileName, dbCtx, fstate, userName,
				shareName, userId);
		if (finfo == null) {
			log4j.warn("DBD#getStreamList() getFileDetails  finfo==null; fileName="
		...
		StreamInfo sinfo = new StreamInfo("::$DATA", finfo.getFileId(), 0,
				finfo.getSize(), finfo.getAllocationSize());
	 ...
	 * @exception IOException
	 */
	protected final NetworkFile createStream(SrvSession sess,
			TreeConnection tree, FileOpenParams params, FileState parent,
			DBDeviceContext dbCtx) throws IOException {
		...
		if (finfo == null) {
			log4j.warn("DBD#createStream() AccessDeniedException  parent:"
					+ parent);
			throw new AccessDeniedException();
		}
		...
		StreamInfoList streamList = (StreamInfoList) parent
				.findAttribute(DBStreamList);
		if (streamList == null)
		...
		if (streamList.findStream(params.getStreamName()) != null) {
			...
			throw new FileExistsException("Stream exists, "
					+ params.getFullPath());
		}
		...
		try {
			...
			int stid = dbCtx.getDBInterface().createStreamRecord(
					params.getStreamName(), finfo.getFileId());

			// Create a network file to hold details of the new stream

			file = (DBNetworkFile) dbCtx.getFileLoader().openFile(params,
					finfo.getFileId(), stid, finfo.getDirectoryId(), true,
					false, userName);
			...
			StreamInfo stream = new StreamInfo(params.getStreamName(),
					file.getFileId(), stid);
			streamList.addStream(stream);

			// DEBUG
			log4j.debug("DBD#createStream() file=" + params.getPath()
					+ ", stream=" + params.getStreamName() + ", fid/stid="
					+ file.getFileId() + "/" + stid);
		} catch (DBException ex) {
			log4j.error("Error: " + ex.toString());
		} finally {

			// Check if the stream file is not valid but an access token has
			// been allocated
		...
		if (file == null) {
			log4j.error("AccessDeniedException ,fullPath:"
					+ params.getFullPath());
	 ...
	 * @exception IOException
	 */
	protected final NetworkFile openStream(SrvSession sess,
			TreeConnection tree, FileOpenParams params, FileState parent,
			DBDeviceContext dbCtx) throws IOException {
		...
		if (finfo == null) {
			log4j.warn("DBD#openStream() AccessDeniedException  parent:"
					+ parent);
			throw new AccessDeniedException();
		}
		...
		if (sInfo == null) {
			log4j.error("FileNotFoundException Stream does not exist, "
					+ params.getFullPath());
			throw new FileNotFoundException("Stream does not exist, "
					+ params.getFullPath());
		}
		...
		FileAccessToken accessToken = null;
		FileState fstate = null;
		log4j.debug("DBD#openStream() file=" + params.getPath() + ", stream="
				+ sInfo.getName());
		try {
			...
			if (params.getSharedAccess() == SharingMode.NOSHARING
					&& fstate.getOpenCount() > 0
					&& params.getProcessId() != fstate.getProcessId())
				throw new FileSharingException("File already open, "
						+ params.getPath());

			// Set the file information for the stream, using the stream
			// information

			DBFileInfo sfinfo = new DBFileInfo(sInfo.getName(),
					params.getFullPath(), finfo.getFileId(),
			...
			jdbcFile = (DBNetworkFile) dbCtx.getFileLoader().openFile(params,
					finfo.getFileId(), sInfo.getStreamId(),
	 ...
	 * @exception IOException
	 */
	protected final void closeStream(SrvSession sess, TreeConnection tree,
			NetworkFile stream) throws IOException {

		// Debug
		log4j.debug("DBD#closeStream() file=" + stream.getFullName()
				+ ", stream=" + stream.getStreamName() + ", fid/stid="
				+ stream.getFileId() + "/" + stream.getStreamId());
		...
		if (stream instanceof DBNetworkFile) {
			...
			if (jdbcFile.hasAccessToken()) {
				...
				dbCtx.getStateCache().releaseFileAccess(fstate,
						jdbcFile.getAccessToken());
			}
			...
			if (fstate != null) {
				...
				FileInfo finfo = getFileInformation(sess, tree,
						fstate.getPath());
				log4j.debug("DBDiskDriver.closeStream ; path:"
						+ fstate.getPath() + " , fstate:" + fstate);
				if (finfo != null && stream.getWriteCount() > 0) {
					...
					// DEBUG
					log4j.debug("  Stream size=" + jdbcFile.getFileSize()
							+ ", modifyDate=" + jdbcFile.getModifyDate());
				}
			}
		} else
		...
		// Check if the stream was opened for write access, if so then update
		// the stream size

		if (stream.getGrantedAccess() != NetworkFile.READONLY
				&& stream.isDirectory() == false && stream.getWriteCount() > 0) {
			...
			String parentPath = FileName.getParentPathForStream(stream
					.getFullName());
			...
			if (parent != null) {
				...
				StreamInfoList streamList = getStreamList(sess, tree,
						parentPath);
				if (streamList != null) {
					...
					if (sInfo != null) {
						...
						log4j.error("** Failed to find details for stream "
								+ stream.getStreamName());
				} else
					log4j.error("** Failed to get streams list for "
							+ parentPath);
			}
			...
			try {
				...
				dbCtx.getDBInterface().setStreamInformation(
						stream.getDirectoryId(), stream.getFileId(),
	 ...
	 * @exception IOException
	 */
	protected final void deleteStream(SrvSession sess, TreeConnection tree,
			String name) throws IOException {
		...
		DBDeviceContext dbCtx = (DBDeviceContext) tree.getContext();
		String shareName = DiskUtil.findFristPath(name, tree.getFullName());
		log4j.debug("DBD#deleteStream ; name:" + name + " , shareName:"
				+ shareName + ", userName:" + userName);
		...
		DBFileInfo finfo = getFileDetails(filePath, dbCtx, fstate, userName,
				shareName, userId);
		...
		try {
			...
			dbCtx.getFileLoader().deleteFile(name, sInfo.getFileId(),
					sInfo.getStreamId(), shareName);

			// Delete the stream record

			dbCtx.getDBInterface().deleteStreamRecord(sInfo.getFileId(),
					sInfo.getStreamId(), dbCtx.isTrashCanEnabled());
	 ...
	 * @return StreamInfoList
	 */
	protected final StreamInfoList loadStreamList(FileState fstate,
			DBFileInfo finfo, DBDeviceContext dbCtx, boolean dbLoad) {

		// Check if the stream list has already been loaded

		StreamInfoList sList = (StreamInfoList) fstate
				.findAttribute(FileState.StreamsList);
		...
		if (sList == null && dbLoad == true) {
			...
			try {
				...
				sList = dbCtx.getDBInterface().getStreamsList(
						finfo.getFileId(), DBInterface.StreamAll);
	 ...
	 * @exception IOException
	 */
	public void renameStream(SrvSession sess, TreeConnection tree,
			String oldName, String newName, boolean overWrite)
			throws IOException {
	}
	 ...
	 * @exception FileNotFoundException
	 */
	public String buildPathForFileId(SrvSession sess, TreeConnection tree,
			int dirid, int fileid) throws FileNotFoundException {

		// Access the JDBC context
		String userName = sess.getClientInformation().getUserName();
		String shareName = tree.getContext().getShareName();
		log4j.error("\n\n\nDBD#buildPathForFileId ; shareName 是错误的！需要改, shareName:"
				+ shareName + ", userName:" + userName + "\n\n\n");
		...
		log4j.debug("DBD#buildPathForFileId ; dirid:" + dirid + " , fileid:"
				+ fileid + " , shareName:" + shareName + ", userName:"
				+ userName);
		try {
			...
			do {
				...
				finfo = dbCtx.getDBInterface().getFileInformation(curDid,
						curFid, DBInterface.FileIds, userId, shareName,
						userName);
	...
	public boolean hasSymbolicLinksEnabled(SrvSession sess, TreeConnection tree) {
		...
		// Access the associated database interface to check if it supports
		// symbolic links
	 ...
	 * @exception FileNotFoundException
	 */
	public String readSymbolicLink(SrvSession sess, TreeConnection tree,
			String path) throws AccessDeniedException, FileNotFoundException {
		...
		if (dbInterface.supportsFeature(DBInterface.FeatureSymLinks)) {
			...
			if (fid == -1) {
				...
				dirId = findParentDirectoryId(dbCtx, path, true, userName,
						shareName, userId);
				if (dirId == -1) {
					log4j.warn("DBD#readSymbolicLink() FileNotFoundException  path:"
							+ path);
				...
				fid = getFileId(path, fname, dirId, dbCtx, userId, shareName,
						userName);
				if (fid == -1) {
			...
			try {
				...
				// Database interface supports symbolic links, read the symbolic
				// link

				symLink = dbInterface.readSymbolicLink(dirId, fid);
			} catch (DBException ex) {
				log4j.warn(
						"DBD#readSymbolicLink() FileNotFoundException  path:"
								+ path, ex);
				throw new FileNotFoundException(path);
			}
		}
	 ...
	 */
	public int getSecurityDescriptorLength(SrvSession sess,
			TreeConnection tree, NetworkFile netFile) throws SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	public SecurityDescriptor loadSecurityDescriptor(SrvSession sess,
			TreeConnection tree, NetworkFile netFile) throws SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	public void saveSecurityDescriptor(SrvSession sess, TreeConnection tree,
			NetworkFile netFile, SecurityDescriptor secDesc)
			throws SMBSrvException {

	}

	public int OfffileExists(SrvSession sess, TreeConnection tree, String name,
			int createOptn) {
		// Access the JDBC context
		String userName = sess.getClientInformation().getUserName();
		int userId = sess.getClientInformation().getUid();
		...
		int fileSts = FileStatus.NotExist;
		FileState fstate = null;//DBUtil.SHARENAME_COMMFILE;
		String shareName = DiskUtil.findFristPath(name, tree.getFullName());
		...
		// 增加根路径的直接返回目录存在(如:username@/我的文件; username@/资料库)
		//
		if (name.equalsIgnoreCase("\\" + DBUtil.SHARENAME_COMMFILE)
				|| name.equalsIgnoreCase("\\" + DBUtil.SHARENAME_USERFILE)
				|| name.equalsIgnoreCase("\\" + DBUtil.SHARENAME_RECIVEFILE)) // 新增加
		{
			// log4j.debug("DBD#fileExists username@\资料库　　根目录 , 直接返回
			// DirectoryExists ");
			return FileStatus.DirectoryExists;
		}
	 ...
	 * @param tree
	 */
	public void reloadFile(String fileVirtualPath, String shareName, TreeConnection tree) {
		DBDeviceContext dbCtx = (DBDeviceContext) tree.getContext();
//		log4j.debug("DBDiskLDriver.reloadFile()   hellno ! ");
//		fileVirtualPath = "\\\\172.20.1.52\\admin@\\资料库\\NAS\\0618\\2010.xlsx";
//		fileVirtualPath = "\\资料库\\NAS\\0618\\a (2).docx";
//		fileVirtualPath = "\\资料库\\NAS\\061821\\nsh6.docx";
//		fileVirtualPath = "\\资料库\\NAS\\061821\\qq.xlsx";
//		shareName = DBUtil.SHARENAME_COMMFILE;
		
		if (shareName.equals("2")) {
			fileVirtualPath = "\\资料库" + fileVirtualPath;
		} else if (shareName.equals("1")){
			fileVirtualPath = "\\我的文件" + fileVirtualPath;
		} else {
			
		}
		
		DBFileLoader loader = (DBFileLoader) dbCtx.getFileLoader();
		
		FileState state = getFileState(fileVirtualPath, dbCtx, false);
		if( state != null ) {
//			loader.fileStateExpired(state);
			state.setExpiryTime(0);
			state.removeAttribute(FileState.FileInformation);// 移除旧的
			
//			state.setFileStatus(FileStatus.FileExists);
//			state.setFileId(finfo.getFileId());
//			state.setFileSize(finfo.getSize());
//			// Attach to the file state
//			state.addAttribute(FileState.FileInformation,
//					finfo);
//			dbCtx.getStateCache().renameFileState(
//					state.getPath(), state, false);// 清理同路径的缓存(非常重要）
			...
			loader.fileStateExpired(state);
			log4j.info("RELOADFILE OK ! " + fileVirtualPath);
		} else {
			log4j.warn("RELOADFILE ERROR ! Maybe it was already removed! " + fileVirtualPath);
		}
		dbCtx.getStateCache().removeFileState(fileVirtualPath);
		...
		if ( tempFile.exists() && tempFile.delete() == false) {
			...
			Debug.println("** Failed to delete " + toString() + " **");
		  	...
		  	//throw new IOException("Failed to delete file " + temporaryFile);
		}
		
		
//		final DBDeviceContext dbCtx = (DBDeviceContext) tree.getContext();
//		log4j.debug("DBDiskLDriver.reloadFile()   hellno ! ");
////		fileVirtualPath = "\\\\172.20.1.52\\admin@\\资料库\\NAS\\0618\\2010.xlsx";
////		fileVirtualPath = "\\资料库\\NAS\\0618\\a (2).docx";
////		fileVirtualPath = "\\资料库\\NAS\\061821\\nsh6.docx";
...
//		}
//		
//		final String fileVirtualPath2 = fileVirtualPath;
//		
//		final DBFileLoader loader = (DBFileLoader) dbCtx.getFileLoader();
//		
//		new Thread() {
//			@Override
//			public void run() {
//				
//				FileState state = getFileState(fileVirtualPath2, dbCtx, false);
//				while (state != null) {
//					loader.fileStateClosed(state);
//					dbCtx.getStateCache().removeFileState(fileVirtualPath2);
//					
//					try {
//						Thread.sleep(3000);
//						log4j.info("RELOADFILE THREAD.SLEEP(3000) !");
//					} catch (InterruptedException e) {
//						// TODO Auto-generated catch block
//						e.printStackTrace();
//					}
//					
//					state = getFileState(fileVirtualPath2, dbCtx, false);
//				}
////				if( state != null ) {
//////					loader.fileStateExpired(state);
////				} else {
////					log4j.debug("RELOADFILE ERROR !");
////				}
//			}
//			
//		}.start();
