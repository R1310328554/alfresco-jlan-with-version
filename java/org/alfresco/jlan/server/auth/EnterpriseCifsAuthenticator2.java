 ...
 * @author gkspencer
 */
public class EnterpriseCifsAuthenticator2 extends CifsAuthenticator implements CallbackHandler {

	private static boolean ldapAuth = false;
	/**
	 * 初始化
	 */
	static
	{
		ldapAuth = DBUtil.ldapAuth;
	}
	// Constants
	//
	...
	public static final String SECURITYKEY = "LINKAPP00USERKEY";
	 ...
	 * Class constructor
	 */
	public EnterpriseCifsAuthenticator2() {
	...
	public void initialize(ServerConfiguration config, ConfigElement params)
		...
		log4j.debug("ECA#initialize config:"+config+",params:"+params);
		...
		if ( params.getChild("disableSessionCleanup") != null) {
			...
			// Debug
			log4j.info("[SMB] Disabled session cleanup (for virtual circuit zero logons)");
		}
        ...
        if ( params.getChild("kerberosDebug") != null) {
        	...
        	log4j.debug("EnterpriseCifsAuthenticator.initialize Enable Kerberos API debug output ");
        }
		...
		if ( krbRealm != null && krbRealm.getValue() != null && krbRealm.getValue().length() > 0) {
			...
			{
				log4j.error("CIFS service account password not specified , srvPassword:"+srvPassword);
			...
			if ( loginEntry != null) {
				...
				{
					log4j.error("Invalid login entry specified, loginEntry:"+loginEntry.getValue());
			...
			if ( principal != null) {
				...
				// Debug
				log4j.debug("\n\ncifsAccount:"+cifsAccount);
				log4j.debug("是否执行到这里  加载被指文件  一  4  m_accountName " +m_accountName);
			}
			else {
				...
				// Debug
				log4j.debug("\n\ncifsAccount:"+cifsAccount);
				log4j.debug("是否执行到这里  加载被指文件  一  5  m_accountName " +m_accountName);
			}

			// Debug
			log4j.debug("[SMB] Using principal - " + m_accountName);
			...
			try {
				...
				// Debug
				log4j.error("[SMB] Failed to login CIFS server service ;", ex);
			...
			log4j.debug("[SMB] Enabling mechTypes :-");
			log4j.debug("       Kerberos5");
			log4j.debug("       MS-Kerberos5");
			...
			try {
				...
				// Debug
				log4j.error("[SMB] Error creating SPNEGO NegTokenInit blob " , ex);
			...
			if ( useSpnego != null) {
				...
				try {
					...
					// Debug
					log4j.error("[SMB] Error creating SPNEGO NegTokenInit blob - " + ex.getMessage());
		...
		// Debug
		log4j.debug("[SMB] Kerberos ticket cracking enabled");
	}
	...
	public void handle(Callback[] callbacks)
		...
		log4j.debug("ECA#handle callbacks:"+callbacks);
	...
	public void generateNegotiateResponse(SMBSrvSession sess, SMBSrvPacket respPkt, boolean extendedSecurity)
		...
		log4j.debug("ECA#generateNegotiateResponse sess:"+sess+" ,respPkt:"+respPkt);
	...
	public void processSessionSetup(SMBSrvSession sess, SMBSrvPacket reqPkt)
		...
		log4j.debug("ECA#processSessionSetup sess:"+sess+" ,reqPkt:"+reqPkt);
		...
		if ( reqPkt.getParameterCount() == 13) {
			...
			try {
				...
				if (m_enableTicketCracking == true)
					log4j.info("processSessionSetup  - doHashedPasswordLogon(sess, reqPkt) ");
				...
				log4j.error("SMBSrvException : "+ex);
				throw ex;
			}
		}
		...
		if ( reqPkt.hasMoreData()) {
			...
			if ( domain == null)
			{
				log4j.error("domain == null ;SMBSrvException , NTInvalidParameter:"+SMBStatus.NTInvalidParameter+", SRVNonSpecificError:"+ SMBStatus.SRVNonSpecificError+" , ErrSrv:"+ SMBStatus.ErrSrv);
				throw new SMBSrvException(SMBStatus.NTInvalidParameter, SMBStatus.SRVNonSpecificError, SMBStatus.ErrSrv);
			}
		}
		...
		if ( reqPkt.hasMoreData()) {
			...
			if ( clientOS == null)
			{
				log4j.error("clientOS=null ; SMBSrvException , NTInvalidParameter:"+SMBStatus.NTInvalidParameter+", SRVNonSpecificError:"+ SMBStatus.SRVNonSpecificError+" , ErrSrv:"+ SMBStatus.ErrSrv);
				throw new SMBSrvException(SMBStatus.NTInvalidParameter, SMBStatus.SRVNonSpecificError, SMBStatus.ErrSrv);
			}
		}

		// DEBUG
		log4j.debug("[SMB] NT Session setup " + (useRawNTLMSSP() ? "NTLMSSP" : "SPNEGO") + ", MID="
		...
		try {
			...
			if ( isNTLMSSP == true) {
				...
				log4j.debug("Process an NTLMSSP security blob ,domain:"+domain);
				respBlob = doNtlmsspSessionSetup(sess, client, buf, secBlobPos, secBlobLen, isUni);
			}
			else {

				// Process an SPNEGO security blob
				log4j.debug("Process an SPNEGO security blob ,domain:"+domain);
...
//		catch (SMBSrvException ex) {
		...
		// Debug
			...
			if ( respBlob == null)
				log4j.debug("[SMB] User " + client.getUserName() + " logged on "
						+ (client != null ? " (type " + client.getLogonTypeString() + ")" : ""));
			else
				log4j.debug("[SMB] Two stage logon (" + (isNTLMSSP ? "NTLMSSP" : "SPNEGO") + ")");
		...
		if ( respBlob != null || sess.hasSetupObject(client.getProcessId()) || setupObj != null) {
			...
			if ( reqLen > respPkt.getAvailableLength()) {
				...
				try {
					...
					// DEBUG
					log4j.error("Authenticator failed to allocate packet from pool, reqSiz="
								+ (respPkt.getByteOffset() + respLen),ex);
	...
	private final byte[] doNtlmsspSessionSetup(SMBSrvSession sess, ClientInfo client, byte[] secbuf, int secpos, int seclen,
		...
		log4j.debug("ECA#doNtlmsspSessionSetup sess:"+sess+" ,client:"+client);
		...
		if ( msgType == -1) {
			...
			// DEBUG
			log4j.info("[SMB] Invalid NTLMSSP token received");
			log4j.info("[SMB]   Token=" + HexDump.hexString(secbuf, secpos, seclen, " "));
		...
		// Check for a type 1 NTLMSSP message
			...
			if ( sess.hasSetupObject(client.getProcessId()) == false
				...
				log4j.error(" logon failure,SMBSrvException ");
				throw new SMBSrvException(SMBStatus.NTLogonFailure, SMBStatus.ErrDos, SMBStatus.DOSAccessDenied);
			}
...
//TODO
					...
					// Debug
					log4j.debug("[SMB] Logged on using NTLMSSP/NTLMv2");
				}
				else {
					...
					// Debug
					log4j.debug("[SMB] Logged on using NTLMSSP/NTLMv2SessKey");
				}
			}
			else {
				...
				// Debug
				log4j.debug("[SMB] Logged on using NTLMSSP/NTLMv1");
			}
		}
	...
	private final byte[] doSpnegoSessionSetup(SMBSrvSession sess, ClientInfo client, byte[] secbuf, int secpos, int seclen,
			boolean unicode)
		throws SMBSrvException {
		log4j.debug("ECA#doSpnegoSessionSetup sess:"+sess+" ,client:"+client);
		...
		if ( tokType == SPNEGO.NegTokenTarg && sess.hasSetupObject(client.getProcessId())
			...
			log4j.debug("[SMB] Unknown SPNEGO token type");
		...
		try {
			...
			// Debug
			log4j.error("[SMB] Failed to encode NegTokenTarg - " + ex.getMessage());
	...
	private final NegTokenTarg doKerberosLogon(SMBSrvSession sess, NegTokenInit negToken, ClientInfo client)
		...
		log4j.debug("ECA#NegTokenTarg sess:"+sess+" ,client:"+client);
        ...
        try
        {
        	...
        	log4j.debug( "[SMB] Kerberos AP-REQ - " + krbApReq);
        	...
        	if ( krbApReq.hasMutualAuthentication() && m_enableTicketCracking == true)
        	{
        		...
        		// DEBUG
        		log4j.debug("[SMB] Kerberos mutual auth required, parsing AP-REQ");
        		...
        		catch ( IOException ex)
        		{
        			...
        			log4j.debug("[SMB] Failed to parse AP-REQ, " + ex.toString());
            ...
            if ( result != null)
            {
                ...
                if ( negToken.hasOid( OID.MSKERBEROS5))
                {
                	...
                	// DEBUG
                	log4j.debug("[SMB] Using OID MS Kerberos5 for NegTokenTarg");
                }
                else
                {
                	...
                	// DEBUG
                	log4j.debug("[SMB] Using OID Kerberos5 for NegTokenTarg");
                }
                ...
                if ( krbAuthCtx != null)
                {
	                try
	                {
	                	...
	                	// DEBUG
		                log4j.debug("[SMB] Created NegTokenTarg using updated AP-REP, added subkey");
	                }
	                catch (Exception ex)
	                {
	                	log4j.error("[SMB] AP-REP Error:",ex);
	                }
                }
                else
                {
	                ...
	                // DEBUG
	                log4j.debug("[SMB] Created NegTokenTarg using standard Krb5 API response");
                }
                
            	// Check if this is a null logon
            	
            	String userName = krbDetails.getUserName();
            	log4j.debug("krbDetails.getUserName()=" + krbDetails.getUserName() + ",client.getUserName()="+client.getUserName());
            	...
            	if ( userName != null)
            	{
            		...
            		if ( userName.endsWith( "$") && userName.equalsIgnoreCase(userName))
            		{
            			// Null logon
                		...
                		//  Debug
                		log4j.debug("[SMB] Machine account logon, " + userName + ", as null logon");
            		}
            		else
            		{
                ...
                //  Debug
                log4j.debug("[SMB] Logged on using Kerberos, user " + userName);
            }
            else
            {
            	// Debug
            	log4j.info( "[SMB] No SPNEGO response, Kerberos logon failed");
        ...
        {
            // Log the error
        	log4j.error("[SMB] Kerberos logon error",ex);
            // Return a logon failure status
	...
	private final void doNTLMv1Logon(SMBSrvSession sess, ClientInfo client, Type3NTLMMessage type3Msg)
		...
		if ( acceptNTLMv1Logon() == false) {
			...
			log4j.info("[SMB] NTLMv1 not accepted, client " + sess.getRemoteName());
		...
		log4j.debug("doNTLMv1Logon: type3Msg.getUserName():"+type3Msg.getUserName()+", client.getUserName()="+client.getUserName());
		...
		if ( Debug.EnableInfo && hasDebug())
			log4j.debug("此处cifs 登录验证  NTLMSSP/NTLMv1   ; username="+userName);
		// Get the user details

		UserAccount user = getUserDetails1(userName);
...
//			client.setGuest(sts == AUTH_GUEST ? true : false);
			...
			log4j.error("[SMB] doNTLMv1Logon:User does not exist, " + userName);
	...
	private final void doNTLMv1Logon(SMBSrvSession sess, ClientInfo client)
		...
		if ( acceptNTLMv1Logon() == false) {
			...
			// NTLMv1 password hashes not accepted
			log4j.warn("[SMB] NTLMv1 not accepted, client " + sess.getRemoteName());
		...
		if ( user != null) {
			...
			int sts = authenticateUser(client, sess, NTLM1,user);
			...
			// Log a warning, user does not exist
			log4j.error("[SMB] doNTLMv1Logon(SMBSrvSession sess, ClientInfo client):User does not exist, " + userName);
	...
	private final void doNTLMv2Logon(SMBSrvSession sess, ClientInfo client, Type3NTLMMessage type3Msg)
		...
		String userName = type3Msg.getUserName();
		log4j.debug("doNTLMv2Logon: type3Msg.getUserName():"+type3Msg.getUserName()+", client.getUserName()="+client.getUserName());
		...
		log4j.debug("此处cifs 登录验证  NTLMSSP/NTLMv2   ; username="+userName);
		// Get the user details
			//不采用ldap验证
  if(!ldapAuth){
		...
		if ( user != null) {
			...
			try {
				...
				//密码 2
				String u_pwd2 = AESUtil.encryptStrToHexStr(user.getPassword(), SECURITYKEY).trim();
				byte[] md4Pwd2 = null;
				...
				if ( user.hasMD4Password())
					...
					md4Pwd2 = getEncryptor().generateEncryptedPassword(u_pwd2, type2Msg.getChallenge(),PasswordEncryptor.MD4, null, null);
				}
				...
				//密码 2
				byte[] v2hash2 = getEncryptor().doNTLM2Encryption(md4Pwd2, type3Msg.getUserName(), type3Msg.getDomain());
				...
				//密码 2
				byte[] srvHmac2 = v2blob.calculateHMAC(srvChallenge, v2hash2);
...
//							throw new SMBSrvException(SMBStatus.NTLogonFailure, SMBStatus.ErrDos, SMBStatus.DOSAccessDenied);
						...
						int j = 0;
						while (j < clientHmac.length && clientHmac[j] == srvHmac2[j])
							j++;
						...
						{
							user.setMD4Password(md4Pwd2);//重新设置md4Pwd，因为使用的是加密密码
						}
					}
				}
			...
			// Log a warning, user does not exist
			log4j.info("[SMB] doNTLMv2Logon:User does not exist, " + userName);
	...
	else
	{
		...
		if ( user != null) {
			...
			// Log a warning, user does not exist
			log4j.info("[SMB] doNTLMv2Logon.getUserDetails2 ldapAuth=true:User does not exist, " + client.getUserName());
	...
	private final void doNTLMv2Logon(SMBSrvSession sess, ClientInfo client)
		...
		// Check for a null logon
		log4j.debug("ECA#doNTLMv2Logon sess:"+sess+" ,client:"+client);
	...
	if(!ldapAuth){
		...
		UserAccount user = getUserDetails1(client.getUserName());
		...
		if ( user != null) {
			...
			// Log a warning, user does not exist
			log4j.warn("[SMB] doNTLMv2Logon.getUserDetails1:User does not exist, " + client.getUserName());
			...
			UserAccount user = getUserDetails2(client.getUserName());
			if ( user != null) {
				...
				// Log a warning, user does not exist
				log4j.warn("[SMB] doNTLMv2Logon.getUserDetails2:User does not exist, " + client.getUserName());
	...
	private final void doNTLMv2SessionKeyLogon(SMBSrvSession sess, ClientInfo client, Type3NTLMMessage type3Msg)
		...
		String userName = type3Msg.getUserName();
		log4j.debug("doNTLMv2SessionKeyLogon: type3Msg.getUserName():"+type3Msg.getUserName()+", client.getUserName()="+client.getUserName());
		...
		// Get the user details
		log4j.debug("此处cifs 登录验证  NTLMSSP/NTLMv2SessKey ; ldapAuth:"+ldapAuth+", username="+userName+", client:"+client.toString());
			//不采用ldap验证
	if(!ldapAuth){
			UserAccount user = getUserDetails1(userName);
		if ( user != null) {
			...
			try {
				...
				// Log the error
				log4j.error("doNTLMv2SessionKeyLogon arraycopy .NoSuchAlgorithmException :",ex);
			...
			if ( user.hasMD4Password())
				...
				try {
					md4byts = getEncryptor().generateEncryptedPassword(user.getPassword(), null, PasswordEncryptor.MD4, null,
							null);
					md4byts2 = getEncryptor().generateEncryptedPassword(u_pwd2, null, PasswordEncryptor.MD4, null,null);
				}
				catch (Exception ex) {					
					// DEBUG
					log4j.error("doNTLMv2SessionKeyLogon generateEncryptedPassword .Exception:",ex);
					// Return a logon failure
			...
			//密码2
			System.arraycopy(md4byts2, 0, p21_2, 0, 16);

			// Generate the local hash of the password

			byte[] localHash = null;
			//密码2
			byte[] localHash2 = null;

			try {
				localHash = getEncryptor().doNTLM1Encryption(p21, v2challenge);
				
				localHash2 = getEncryptor().doNTLM1Encryption(p21_2, v2challenge);
			}
			catch (NoSuchAlgorithmException ex) {

				// Log the error
				log4j.error("doNTLMv2SessionKeyLogon doNTLM1Encryption .NoSuchAlgorithmException:",ex);
			}
...
//					throw new SMBSrvException(SMBStatus.NTLogonFailure, SMBStatus.ErrDos, SMBStatus.DOSAccessDenied);
					...
					/* 密码2验证   当第一种规划验证失败后，使用第二种规则(即返回给客户端的加密密码) start */
					log4j.debug("改用 密码 2(加密密码)认证 ;i="+i+",length:"+ clientHash.length+" , clientHash:"+bytesToHexString(clientHash)+", localHash:"+bytesToHexString(localHash));
					int j = 0;
					while (j < clientHash.length && clientHash[j] == localHash2[j])
						j++;
					
					if(j!= clientHash.length)
					{
						// Return a logon failure
						log4j.debug("使用了 密码 2验证失败!!! ; j="+j+",length:"+ clientHash.length+" , clientHash:"+bytesToHexString(clientHash)+", localHash2:"+bytesToHexString(localHash2));
						throw new SMBSrvException(SMBStatus.NTLogonFailure, SMBStatus.ErrDos, SMBStatus.DOSAccessDenied);
					}
					/* 密码2验证 end */
				}
			...
			// Log a warning, user does not exist
			log4j.warn("[SMB] doNTLMv2SessionKeyLogon.getUserDetails1:User does not exist, " + userName);
			//新增下面两行
			client.setLogonType(ClientInfo.LogonNull);
			...
			if ( user != null) {
				...
				// Log a warning, user does not exist
				log4j.warn("[SMB] doNTLMv2SessionKeyLogon.getUserDetails2:User does not exist, " + userName);
				//新增下面两行
				client.setLogonType(ClientInfo.LogonNull);
				sess.setLoggedOn(false);
				// Return a logon failure
	...
	private final void doHashedPasswordLogon(SMBSrvSession sess, SMBSrvPacket reqPkt)
		throws SMBSrvException {
		log4j.debug("ECA#doHashedPasswordLogon sess:"+sess);
		// Check that the received packet looks like a valid NT session setup andX request
		...
		// Debug
		log4j.debug("[SMB] NT Session setup from user=" + user + ", password="
		...
		// Debug
		log4j.debug("[SMB]   MID=" + reqPkt.getMultiplexId() + ", UID=" + reqPkt.getUserId() + ", PID="
					+ reqPkt.getProcessId());
		...
		if ( uniPwd != null) {
			if ( uniPwd.length == 24) {
				...
				// Debug
				log4j.debug("[SMB] Logged on using Hashed/NTLMv1 ,client:"+client.toString());
			}
				...
				// Debug
				log4j.debug("[SMB] Logged on using Hashed/NTLMv2 ,client:"+client.toString());
			}
		}
		else {
		...
		if ( client.isGuest()) {
			...
			// DEBUG
			log4j.debug("[SMB] User " + user + ", logged on as guest ,client:"+client.toString());
		}
	...
	public UserAccount getUserDetails1(String name){
		...
		if ("anonymous".equals(name)) {
			account = new UserAccount(name,name);
		...
		try {
			...
			{
				log4j.warn("getUserDetails1 by userName not exist!");
			}
		}catch (Exception ex) {
			log4j.error(ex);
		}
