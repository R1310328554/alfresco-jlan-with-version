...
public class DBConnectionPool {
    ...
    public final static int MaximumConnections = 2000;

    //	Default pool size

    public final static int DefaultMinSize = 5;
    public final static int DefaultMaxSize = 500;
                    ...
                    while (enm.hasMoreElements()) {
                        ...
                        if (expire.longValue() != PermanentLease && expire.longValue() < timeNow)
                            ...
                            try {
                                ...
                                ex.printStackTrace();
                                removeConn = true;
                                from = "con is exception";
                            }

                            // DEBUG
                            if (removeConn == true)
                                Debug.println("DBConnectionReaper Permanent lease connection error\t."+from+"\t.expire.longValue="+expire.longValue());
                        }
                        ...
                        if (removeConn == true) {
                            ...
                            try {
                                conn.close();
                                Debug.println("DBConnectionReaper closed expired connection, conn=" + conn + "\t.expire.longValue="+expire.longValue());
                            } catch (SQLException ex) {
                            }
                        }
                    }
