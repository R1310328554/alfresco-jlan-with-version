...
public class CifsOnlyXMLServerConfiguration extends ServerConfiguration {
	...
	private Logger log4j = Logger.getLogger(this.getClass());
	  ...
	  private CifsAuthenticator authtor = null;
	...
	public String getXmlpath()
	  ...
	  public void setXmlpath(String xmlpath) {
	    this.xmlpath = FileUtil.getRootPath()+"/WEB-INF/classes/jlanConfig.xml";
	    try {
	      loadConfiguration(xmlpath);
	    } catch (Exception ex) {
	      ex.printStackTrace();
	    }
	  }
...
//	private static final int MemoryPoolMaximumAllocation    = 500;
	...
	public final void loadConfiguration(String fname)
		...
		// Open the configuration file
		  if ((!("".equals(this.xmlpath))) && (this.xmlpath != null)) {
		      fname = this.xmlpath;
		    }
		InputStream inFile = new FileInputStream(fname);
		Reader inRead = new InputStreamReader(inFile,"UTF-8");//必须指定编码，否则有中文时有些操作系统会报错
		log4j.debug("************ loading config file ************  " + fname);
...
//			if (authType.equalsIgnoreCase("enterprise")) {
//			     log4j.debug("************* 启用SPRING 用户认证 *****************");
//
//			 cifsConfig.setAuthenticator(getAuthtor(), params, accessMode, allowGuest != null);
//			    }
//			    else {
...
//			    }
	...
	protected final void procSecurityElement(Element security)
		...
		Element model = findChildNode("model", security.getChildNodes()); 
		if (model != null) {
			      if ("enterprise".equals(model.getTextContent())) {
			    	 log4j.debug(" ************ 启用   NAS企業級  验证  ************");

			        secConfig.setM_aclManager(getAccessControlManager());
			      }

			    }
		else if ( aclElem != null) {
...
//			SharedDeviceList shareList = shareMapper.getShareList(getServerName(), null, false);
	...
	protected final AccessControlList procAccessControlElement(Element acl, SecurityConfigSection secConfig)
		...
		log4j.debug(" ************ accessManager ************" + secConfig.getAccessControlManager());
	...
	protected final void addDiskShare(Element disk, FilesystemsConfigSection filesysConfig)
		...
		if ( sizeElem != null) {
			...
			if ( totSize == -1L)
			{
				log4j.error("xmlConfig# Total disk size invalid or not specified");
			...
			if ( freeSize == -1L)
			{
				log4j.error("xmlConfig# Free disk size invalid or not specified");
			...
			if ( attr != null && attr.length() > 0) {
				try {
					...
					{
						log4j.error("xmlConfig# Block size must be a multiple of 512");
						throw new InvalidConfigurationException("Block size must be a multiple of 512");
					}
				}
				catch (NumberFormatException ex) {
					log4j.error("xmlConfig# Invalid block size specified, " + attr);
			...
			if ( attr != null && attr.length() > 0) {
				try {
					...
					if ( blocksPerUnit <= 0)
					{
						log4j.error("xmlConfig# Invalid blocks per unit, must be greater than zero");
						throw new InvalidConfigurationException("Invalid blocks per unit, must be greater than zero");
					}
				}
				catch (NumberFormatException ex) {
					log4j.error("xmlConfig# Invalid blocks per unit value");
	...
	public CifsAuthenticator getAuthtor() {
		return authtor;
	}

	public void setAuthtor(CifsAuthenticator authtor) {
	...
	public void setShareMapper(ShareMapper shareMapper) {
	  ...
	  public void loadDBShare(FilesystemsConfigSection config) {
