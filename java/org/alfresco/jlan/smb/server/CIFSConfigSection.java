...
public class CIFSConfigSection extends ConfigSection {
  ...
  public void setAuthenticator(CifsAuthenticator auth, ConfigElement params, int accessMode, boolean allowGuest)
  	throws InvalidConfigurationException
	{
	  int sts = 0;
	  System.out.println(auth);
	
	  if (auth instanceof CifsAuthenticator) {
	    System.out.println("DIY  AUTH.....");
	    this.m_authenticator = auth;
	    auth.setAccessMode(accessMode);
	    auth.setAllowGuest(allowGuest);
	  }
	  else {
	    throw new InvalidConfigurationException("Authenticator is not derived from required base class11111");
	  }
	
	  auth.initialize(getServerConfiguration(), params);
	  sts = fireConfigurationChange(accessMode,auth);
	}
